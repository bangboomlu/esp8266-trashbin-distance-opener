...
int pulselength_oneRotation_ccw_perSecond = 1565;// <-|these two values are depending on your servo. For the FS90R 1500 Microseconds should be not turning at all
int pulselength_oneRotation_cw_perSecond = 1365;// <-|less than 1500 turns the servo right and more is turning left. The further the value away from 1500 the faster it turns.
long duration=0;
long distance;
int i;
Servo myservo;
...
{     
    ...
    if (distance >= 20 && state!=1) // if distance >= 20cm turn servo one rotation left
        {
            myservo.writeMicroseconds(pulselength_oneRotation_ccw_perSecond);// this value is depending on your servo
            delay(1000);
            state=1;
        }
    else if (distance <= 21 && state!=2)
        {
            myservo.writeMicroseconds(pulselength_oneRotation_cw_perSecond);
            delay(1000);
            state=2;
        }
